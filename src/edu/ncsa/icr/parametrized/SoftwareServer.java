...
import edu.ncsa.icr.parametrized.ICRAuxiliary.*;
import java.io.*;
import java.net.*;
import java.util.*;
...
public class SoftwareServer implements Runnable
{
   ...
   */
  public synchronized String executeTask(String host, int session, Vector<Subtask> task, Vector<String> task_params)
  {
  	Subtask subtask;
  	Application application;
  	...
  	//Execute each subtask in the task
		for(int i=0; i<task_attempts; i++){
	  	for(int j=0; j<task.size(); j++){
	  		...
	  		subtask_params="";
	  		if(task_params!=null)
	  			subtask_params=task_params.get(j);
	  		if(subtask_params==null)
	  			subtask_params="";
//	  		System.out.println(subtask_params);
...
//	ICRAuxiliary.Application.print(applications);
  	for(int i=0; i<applications.size(); i++){
...
//  		if(true){
//  			for(Operation executable : applications.get(i).operations){
//  				System.out.println("    " + executable);
//  			}
//  		}
		...
		try{
			...
			while(client_socket.isConnected()){
				...
				if(message.equals("send")){
					...
					executeTask(host, session, task, null);
			...
			if(args[0].equals("-test")){
				...
				for(int i=0; i<server.applications.size(); i++){
					...
					if(input_operation != -1 && output_operation != -1){
						...
						server.executeTask("localhost", i, task,null);	//Use application index as the session
